...
import { FileText, Clock, Plus, Trash, PencilSimple, Eye, X, Sun, Moon as MoonIcon, Coffee, Lightning, Users, Bell, PiggyBank, Calculator, Receipt, ChartBar, SpinnerGap, Target, UserPlus, CheckCircle, Info, EnvelopeSimple, Copy, PaperPlaneTilt, GearSix } from '@phosphor-icons/react';
...
const PLATFORMS = [
  { id: 'wise', name: 'Wise', icon: '/icons/wise.svg', emoji: '💸' },
  { id: 'payoneer', name: 'Payoneer', icon: '/icons/payoneer.svg', emoji: '🔵' },
  { id: 'paypal', name: 'PayPal', icon: '/icons/paypal.svg', emoji: '🅿️' },
...
const LEAD_SOURCES = [
  { id: 'linkedin', name: 'LinkedIn', icon: '/icons/linkedin.mid.svg', emoji: '💼' },
  { id: 'onlinejobsph', name: 'OnlineJobsPH', icon: '🇵🇭', emoji: '🇵🇭' },
  { id: 'upwork', name: 'Upwork', icon: '/icons/upwork.svg', emoji: '🟢' },
  { id: 'facebook', name: 'Facebook', icon: '/icons/facebook.mid.svg', emoji: '📘' },
  { id: 'tiktok', name: 'TikTok', icon: '/icons/tiktok.svg', emoji: '🎵' },
  { id: 'instagram', name: 'Instagram', icon: '/icons/instagram.svg', emoji: '📸' },
  { id: 'twitter', name: 'Twitter/X', icon: '/icons/x.svg', emoji: '🐦' },
  { id: 'fiverr', name: 'Fiverr', icon: '/icons/fiverr.svg', emoji: '🟩' },
...
const renderIcon = (icon, size = 20, fallback = '🌐') => {
  if (icon && (icon.startsWith('http') || icon.startsWith('/icons/'))) {
    return (
      <img
        src={icon}
        alt=""
...
const VAKita = ({ user, isDark }) => {
  ...
  // Adjust breakpoints to account for sidebar width (~260px)
  // This ensures desktop layouts when sidebar is visible
  const isMobile = width < 1024;  // Was 768 - now accounts for sidebar
  const isSmall = width < 768;    // Was 480 - now accounts for sidebar
...
${senderName}`;
  ...
  if (loading) return (<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '400px', flexDirection: 'column', gap: '16px' }}><SpinnerGap style={{ width: '32px', height: '32px', color: '#004AAC', animation: 'spin 1s linear infinite' }} /><p style={{ fontSize: '14px', color: theme.textMuted }}>Loading VAKita...</p><style>{'@keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }'}</style></div>);
    ...
    <>
      ...
      <div style={{ backgroundColor: isDark ? '#0a0a0b' : '#ffffff', borderBottom: '1px solid ' + theme.cardBorder }}>
        ...
        <div style={{
          ...
          {!isMobile && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
              ...
              >
                <GearSix style={{ width: '18px', height: '18px' }} />
              </button>
              ...
              {saving && <span style={{ fontSize: '12px', color: theme.textMuted, display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}><SpinnerGap style={{ width: '12px', height: '12px', animation: 'spin 1s linear infinite' }} />Saving...</span>}
            </div>
          )}
        </div>
        ...
        {isMobile && (
          <div style={{
            ...
            {saving && <span style={{ fontSize: '12px', color: theme.textMuted, display: 'flex', alignItems: 'center', gap: '4px' }}><SpinnerGap style={{ width: '12px', height: '12px', animation: 'spin 1s linear infinite' }} /></span>}
          </div>
        )}
      </div>
      ...
      {['prospecting', 'clients', 'invoices', 'dashboard'].includes(activeTab) && (
        <div style={{
          ...
          <div style={{ margin: '0', padding: isSmall ? '10px 8px' : '12px 16px' }}>
            <div style={{ display: 'flex', gap: isSmall ? '4px' : '6px', backgroundColor: theme.toggleBg || (isDark ? '#18181b' : '#f4f4f5'), borderRadius: '8px', padding: '4px', width: isSmall ? '100%' : 'fit-content', overflow: 'auto' }}>
              ...
              >
                <ChartBar style={{ width: isSmall ? '12px' : '14px', height: isSmall ? '12px' : '14px' }} />
                Income
              </button>
            </div>
          </div>
      ...
      <div style={{ padding: isSmall ? '12px 8px' : '24px' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: isSmall ? 'column' : 'row', justifyContent: 'space-between', alignItems: isSmall ? 'stretch' : 'flex-start', gap: '12px' }}>
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap', justifyContent: isSmall ? 'space-between' : 'flex-end' }}>
                          ...
                          <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                            ...
                            <button onClick={() => { setEditingProspect(prospect); setProspectForm({ ...prospect, estimatedValue: prospect.estimatedValue?.toString() || '' }); setShowProspectForm(true); }} style={{ ...btnGhost, width: isSmall ? '32px' : '36px', height: isSmall ? '32px' : '36px', padding: 0 }}><PencilSimple style={{ width: '14px', height: '14px' }} /></button>
                            <button onClick={() => setDeleteModal({ show: true, type: 'prospect', id: prospect.id, name: prospect.name })} style={{ ...btnGhost, width: isSmall ? '32px' : '36px', height: isSmall ? '32px' : '36px', padding: 0, color: '#ef4444' }}><Trash style={{ width: '14px', height: '14px' }} /></button>
                          </div>
                        </div>
                      </div>
                    </div>
                        ...
                        <div key={prospect.id} style={{ ...card, padding: '14px', opacity: 0.7 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                              ...
                              <button onClick={() => setDeleteModal({ show: true, type: 'prospect', id: prospect.id, name: prospect.name })} style={{ ...btnGhost, width: '32px', height: '32px', padding: 0, color: '#ef4444' }}><Trash style={{ width: '14px', height: '14px' }} /></button>
                            </div>
                          </div>
                        </div>
                      );
        ...
        {activeTab === 'clients' && (
          <div>
            ...
            {clients.length === 0 ? (<div style={{ ...card, textAlign: 'center', padding: '60px' }}><Users style={{ width: '48px', height: '48px', color: theme.textMuted, margin: '0 auto 16px' }} /><p style={{ fontSize: '16px', color: theme.text }}>No clients yet</p><button onClick={() => setShowClientForm(true)} style={{ ...btnPrimary, marginTop: '16px' }}><Plus style={{ width: '16px', height: '16px' }} />Add Client</button></div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isSmall ? '1fr' : 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
                {clients.map(c => {
                  const tz = TIMEZONES.find(t => t.id === c.timezone); const ts = getStatus(c.timezone, c.businessHoursStart, c.businessHoursEnd); const Icon = ts.icon; const income = incomeEntries.filter(e => e.clientId === c.id).reduce((s, e) => s + (e.phpAmount || 0), 0); const pf = PLATFORMS.find(p => p.id === c.paymentPlatform);
                  return (<div key={c.id} style={{ ...card, borderLeft: '4px solid ' + c.color, opacity: c.status === 'active' ? 1 : 0.6 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}><div><h3 style={{ fontSize: '16px', fontWeight: '600', color: theme.text, margin: 0 }}>{c.name}</h3>{c.company && <p style={{ fontSize: '13px', color: theme.textMuted, margin: '2px 0 0' }}>{c.company}</p>}</div><div style={{ display: 'flex', gap: '4px' }}><button onClick={() => { setEditingClient(c); setClientForm({ ...c, rate: c.rate?.toString() || '' }); setShowClientForm(true); }} style={{ ...btnGhost, width: '32px', height: '32px', padding: 0 }}><PencilSimple style={{ width: '14px', height: '14px' }} /></button><button onClick={() => setDeleteModal({ show: true, type: 'client', id: c.id, name: c.name })} style={{ ...btnGhost, width: '32px', height: '32px', padding: 0, color: '#ef4444' }}><Trash style={{ width: '14px', height: '14px' }} /></button></div></div>
        ...
        {activeTab === 'invoices' && (
          <div>
            ...
            ) : (<div style={{ display: 'flex', flexDirection: 'column', gap: isSmall ? '8px' : '12px' }}>{invoices.map(inv => {
              const isPaid = inv.status === 'paid'; return (<div key={inv.id} style={{ ...card, padding: isSmall ? '12px' : '16px', opacity: isPaid ? 0.85 : 1 }}>
                <div style={{ display: 'flex', flexDirection: isSmall ? 'column' : 'row', justifyContent: 'space-between', alignItems: isSmall ? 'stretch' : 'center', gap: isSmall ? '10px' : '12px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: isSmall ? 'space-between' : 'flex-end' }}>
                    ...
                    <div style={{ display: 'flex', gap: '4px' }}>
                      ...
                      <button onClick={() => setEmailModal({ show: true, invoice: inv })} style={{ ...btnGhost, width: isSmall ? '28px' : '32px', height: isSmall ? '28px' : '32px', padding: 0 }} title="Email"><EnvelopeSimple style={{ width: isSmall ? '14px' : '15px', height: isSmall ? '14px' : '15px' }} /></button>
                      <button onClick={() => { if (!isPaid) { setEditingInvoice(inv); setInvoiceForm({ ...inv }); setShowInvoiceForm(true); } }} style={{ ...btnGhost, width: isSmall ? '28px' : '32px', height: isSmall ? '28px' : '32px', padding: 0, opacity: isPaid ? 0.4 : 1, cursor: isPaid ? 'not-allowed' : 'pointer' }} disabled={isPaid}><PencilSimple style={{ width: isSmall ? '14px' : '15px', height: isSmall ? '14px' : '15px' }} /></button>
                      <button onClick={() => { if (!isPaid) setDeleteModal({ show: true, type: 'invoice', id: inv.id, name: inv.invoiceNumber }); }} style={{ ...btnGhost, width: isSmall ? '28px' : '32px', height: isSmall ? '28px' : '32px', padding: 0, color: '#ef4444', opacity: isPaid ? 0.4 : 1, cursor: isPaid ? 'not-allowed' : 'pointer' }} disabled={isPaid}><Trash style={{ width: isSmall ? '14px' : '15px', height: isSmall ? '14px' : '15px' }} /></button>
                    </div>
                  </div>
                </div>
              </div>);
        ...
        {activeTab === 'timezone' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
              ...
              >
                <button onClick={() => setShowOverlap(!showOverlap)} style={{ ...btnOutline, backgroundColor: showOverlap ? (isDark ? '#22c55e20' : '#22c55e15') : 'transparent', borderColor: showOverlap ? '#22c55e' : theme.cardBorder, color: showOverlap ? '#22c55e' : theme.text }}><Lightning style={{ width: '16px', height: '16px' }} />Find Overlap</button>
                {showOverlapTooltip && !showOverlap && (
                  <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                      <Lightning style={{ width: '16px', height: '16px', color: '#22c55e' }} />
          ...
          <div style={{ width: '100%', maxWidth: '600px', backgroundColor: theme.cardBg, borderRadius: '12px', border: '1px solid ' + theme.cardBorder, maxHeight: '90vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '20px', borderBottom: '1px solid ' + theme.cardBorder, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '40px', height: '40px', backgroundColor: isDark ? '#004AAC20' : '#004AAC10', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <EnvelopeSimple style={{ width: '20px', height: '20px', color: '#004AAC' }} />
                </div>
                <div>
            ...
            <div style={{ padding: '20px', borderTop: '1px solid ' + theme.cardBorder, display: 'flex', gap: '12px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
              ...
              {vakitaProfile.gmailConnected ? (
                ...
                >
                  <PaperPlaneTilt style={{ width: '16px', height: '16px' }} />
                  Send Email
                </button>
              ) : (
                <button
                ...
                >
                  <EnvelopeSimple style={{ width: '16px', height: '16px' }} />
            ...
            <div style={{ padding: '20px', borderBottom: '1px solid ' + theme.cardBorder, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '40px', height: '40px', backgroundColor: isDark ? '#3b82f620' : '#3b82f610', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <PaperPlaneTilt style={{ width: '20px', height: '20px', color: '#3b82f6' }} />
                </div>
                <div>
            ...
            <div style={{ padding: '20px', borderTop: '1px solid ' + theme.cardBorder, display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
              ...
              >
                {sendEmailModal.sending ? (
                  <>
                    <SpinnerGap style={{ width: '16px', height: '16px', animation: 'spin 1s linear infinite' }} />
                    Sending...
                  </>
                ) : vakitaProfile.gmailConnected && vakitaProfile.useGmail ? (
                  <>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 70 }} onClick={() => setShowProfileModal(false)}>
          <div style={{ width: '100%', maxWidth: '480px', maxHeight: '90vh', backgroundColor: theme.cardBg, borderRadius: '12px', border: '1px solid ' + theme.cardBorder, overflow: 'hidden', display: 'flex', flexDirection: 'column' }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: '20px', borderBottom: '1px solid ' + theme.cardBorder, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{ width: '40px', height: '40px', backgroundColor: isDark ? '#004AAC20' : '#004AAC10', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <GearSix style={{ width: '20px', height: '20px', color: '#004AAC' }} />
                </div>
                <div>
            ...
            <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px', overflow: 'auto', flex: 1 }}>
              ...
              <div style={{ borderTop: '1px solid ' + theme.cardBorder, paddingTop: '16px', marginTop: '4px' }}>
                <label style={{ ...label, display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                  <EnvelopeSimple style={{ width: '16px', height: '16px', color: '#ea4335' }} />
                  Gmail Integration
                </label>
